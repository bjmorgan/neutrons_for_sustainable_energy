# Molecular dynamics: observables and neutron connections
...
$$C_{AB}(t) = \frac{1}{T-t} \int_0^{T-t} A(t') B(t' + t) \, dt'$$
...
$$g(\omega) = \int_{-\infty}^{\infty} C_{vv}(t) \, e^{-i\omega t} \, dt$$
...
The **distinct part** $G_d(\mathbf{r}, t)$ tracks where *different* atoms are:

$$G_d(\mathbf{r}, t) = \frac{1}{N} \left\langle \sum_{i \neq j} \delta(\mathbf{r} - [\mathbf{r}_j(t) - \mathbf{r}_i(0)]) \right\rangle$$

At $t = 0$, this equals $g(r) - 1$ (the pair distribution function). As time evolves, it reflects how the structure around each atom changes.
...
$$S(\mathbf{Q}, \omega) = \int \int G(\mathbf{r}, t) \, e^{i(\mathbf{Q} \cdot \mathbf{r} - \omega t)} \, d\mathbf{r} \, dt$$
